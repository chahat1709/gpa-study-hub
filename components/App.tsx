...
const CommandPalette: React.FC<{ onNavigate: (mode: AppMode) => void; onClose: () => void }> = ({
  onNavigate,
  onClose,
}) => {
  ...
  const actions = [
    ...
    },
    {
      ...
      detail: 'Quizzes, papers, and grading',
      icon: Award,
    },
    {
      mode: AppMode.PLANNER,
      label: 'Plan my week',
      detail: 'Tasks, deadlines, and study blocks',
      icon: CalendarDays,
    },
    {
      mode: AppMode.LIBRARY,
      label: 'Find study material',
      detail: 'Notes, resources, and references',
      icon: BookOpen,
    },
    {
      mode: AppMode.TUTOR,
      label: 'Ask the tutor',
      detail: 'Get contextual help with a topic',
      icon: Sparkles,
    },
  ];
    ...
    >
      ...
      >
        <div className="command-heading">
          <div>
            <p className="command-kicker">Quick switcher</p>
        ...
        <div className="command-input-wrap">
          ...
          <input
            ...
            aria-label="Search features or actions"
          />
          <kbd>Esc</kbd>
        </div>
        <div className="command-results">
          {filtered.length ? (
            ...
            <div className="command-empty">
              No matching action. Try “exam”, “plan”, or “library”.
            </div>
          )}
        </div>
        <p className="command-footnote">
          <Command /> K to open anytime <span>·</span> Arrow keys to explore
        </p>
      </section>
    </div>
  );
...
const AppShell: React.FC = () => {
  ...
  const [updateInfo, setUpdateInfo] = useState<null | Awaited<
    ReturnType<typeof updateService.checkForUpdate>
  >>({ version: '', versionCode: 0, apkUrl: '', changelog: '', mandatory: false });
  ...
  const getModeLabel = () => {
    switch (currentMode) {
      ...
      case AppMode.HOMEWORK:
        return 'Scanner';
      case AppMode.ATTENDANCE:
        return 'Attendance Hub';
      case AppMode.EXAM_HUB:
        return 'Exam Hub';
      default:
        return 'Home';
    }
  ...
  const renderContent = () => {
    switch (currentMode) {
      case AppMode.CAMPUS:
        return <CampusInterface onNavigate={setCurrentMode} />;
      case AppMode.TUTOR:
      ...
      <div className="flex-1 flex flex-col h-full relative z-10 overflow-hidden">
        ...
        {!serverOnline && (
          <div className="server-connect-banner flex items-center justify-center gap-2 px-4 py-2 text-xs font-bold shrink-0">
            ...
            <span>
              Running in Offline Mode — All data stored locally. Connect to a college server for
              multi-device sync.
            </span>
          </div>
        )}
        ...
        {updateAvailable && (
          <div className="flex items-center justify-between gap-3 px-4 py-2.5 text-xs font-bold shrink-0 bg-stitch-cyan/15 border-b border-stitch-cyan/30 text-stitch-cyan">
            <div className="flex items-center gap-2">
              ...
              <span>
                Update v{updateInfo?.version} available — {updateInfo?.changelog}
              </span>
            </div>
            <button
        ...
        <header
          className="ui-mobile-header lg:hidden shrink-0 flex items-center justify-between px-4 z-40 sticky top-0 pt-[env(safe-area-inset-top)] box-content transition-all duration-300"
          style={{ height: 52 + insets.top }}
        >
        ...
        <header className="ui-topbar hidden lg:flex h-16 shrink-0 items-center justify-between px-8 z-40">
          <div className="topbar-context">
            <span className="topbar-overline">GTU academic workspace</span>
          ...
          <div className="topbar-actions">
            ...
            <div className={`topbar-status ${isAiActive ? 'is-ready' : 'is-muted'}`}>
              <span />
              {isAiActive ? 'Tutor ready' : 'Tutor setup'}
            </div>
            <NotificationBell />
          </div>
        </header>
        ...
        <main
          className={`ui-main-scroll flex-1 overflow-y-auto overflow-x-hidden native-scroll relative w-full ${!isNavHidden ? `pb-[${bottomNavHeight}px] lg:pb-0` : ''}`}
        >
          <ErrorBoundary>
            <Suspense fallback={<LoadingFallback />}>
              <div
                className={`h-full w-full ${pageTransition === 'exit' ? 'ui-page-exit' : 'ui-page-enter'}`}
              >
                {renderContent()}
              </div>
            </Suspense>
        ...
        {commandOpen && (
          <CommandPalette onNavigate={setCurrentMode} onClose={() => setCommandOpen(false)} />
        )}
      </div>
    </div>
  );
          ...
          <div className="ui-notification-popover absolute right-0 top-full mt-2 w-80 max-h-96 overflow-y-auto bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl z-50">
            ...
            {notifications.length === 0 ? (
              ...
              notifications.slice(0, 20).map(n => (
                <div
                  key={n.id}
                  className={`p-3 border-b border-white/5 ${!n.isRead ? 'bg-white/5' : ''}`}
                >
