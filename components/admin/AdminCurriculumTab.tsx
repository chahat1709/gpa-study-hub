      ...
      <div className="glass-card bg-slate-900/60 p-6 lg:p-8 rounded-[32px] border border-white/10 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="text-center md:text-left">
          ...
          <p className="text-sm text-slate-400 font-medium mt-1">
            Manage active curriculum for current semester.
          </p>
        </div>
        <form onSubmit={handleSubmit} className="flex gap-2 w-full md:w-auto">
          <input
            ...
            className="flex-1 md:w-64 bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-xl px-5 py-3 min-h-[44px] text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500/40"
          />
          <button
            type="submit"
            disabled={!newSubject.trim()}
            className="bg-indigo-600 text-white px-5 py-3 min-h-[44px] rounded-xl font-bold uppercase tracking-wider text-xs hover:bg-indigo-500 transition-colors shadow-lg disabled:opacity-50 flex items-center justify-center"
          >
            <Plus className="w-4 h-4" />
          </button>
        </form>
        {error && (
          <p className="text-xs text-rose-400 font-bold text-center md:text-left">{error}</p>
        )}
      </div>
          ...
          {subjects.map(s => (
            ...
            >
              ...
              <button
                ...
                className="p-3 min-h-[44px] min-w-[44px] text-slate-400 hover:text-rose-400 hover:bg-rose-500/20 rounded-lg transition-colors opacity-100 lg:opacity-0 group-hover:opacity-100 flex items-center justify-center"
              >
