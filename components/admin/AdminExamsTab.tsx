...
import {
  GraduationCap, Sparkles, Trash2, Search, Loader2, Users
} from 'lucide-react';
...
export const AdminExamsTab: React.FC<AdminExamsTabProps> = ({
  subjects, pubTitle, setPubTitle, pubSubject, setPubSubject,
  pubUnit, setPubUnit, pubDuration, setPubDuration,
  pubQuestions, setPubQuestions, isGeneratingQuiz, isPublishingQuiz,
  onGenerateAI, onPublish, publishedQuizzes, onDeleteQuiz,
  classOverview, gradebookSearch, setGradebookSearch,
  gradebookSubjectFilter, setGradebookSubjectFilter, filteredGradebook,
  qText, setQText, qOpt0, setQOpt0, qOpt1, setQOpt1,
  qOpt2, setQOpt2, qOpt3, setQOpt3, qCorrect, setQCorrect,
  qExplanation, setQExplanation, onAddQuestion
}) => (
    ...
    <div className="relative overflow-hidden bg-gradient-to-r from-indigo-900 via-slate-900 to-indigo-950 rounded-[32px] lg:rounded-[40px] p-6 lg:p-10 text-white shadow-2xl">
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <h2 className="text-2xl lg:text-4xl font-black tracking-tight">Class GTU Readiness Overview</h2>
          <p className="text-xs lg:text-sm text-slate-300 max-w-xl font-medium">
            Faculty exam publishing control panel and real-time student GTU exam performance analytics.
          </p>
        </div>
        <div className="bg-white/10 backdrop-blur-md border border-white/20 p-5 rounded-3xl flex items-center gap-5 shrink-0">
          <div className="relative w-16 h-16 flex items-center justify-center">
            <svg className="w-full h-full transform -rotate-90">
              <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="5" className="text-white/20" fill="transparent" />
              <circle cx="32" cy="32" r="28" stroke="currentColor" strokeWidth="5" className="text-emerald-400" fill="transparent"
                strokeDasharray={175} strokeDashoffset={175 - (175 * classOverview.classReadinessIndex) / 100} />
            </svg>
            <span className="absolute text-sm font-black">{classOverview.classReadinessIndex}%</span>
          </div>
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-indigo-200">Overall Readiness</p>
            <p className="text-base font-black text-white">{classOverview.readinessStatus}</p>
            <p className="text-xs text-slate-300 font-medium">{classOverview.totalSubmissions} Student Submissions</p>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 pt-6 border-t border-white/10 relative z-10">
        <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
          ...
          <p className="text-xl lg:text-2xl font-black text-emerald-400 mt-0.5">{classOverview.avgPercentage}%</p>
        </div>
        <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Pass Rate (≥60%)</p>
          <p className="text-xl lg:text-2xl font-black text-indigo-300 mt-0.5">{classOverview.passRate}%</p>
        </div>
        <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Top Subject</p>
          <p className="text-xl lg:text-2xl font-black text-amber-300 mt-0.5 truncate">{classOverview.topSubject}</p>
        </div>
        <div className="bg-white/5 p-4 rounded-2xl border border-white/10">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Published Exams</p>
          <p className="text-xl lg:text-2xl font-black text-white mt-0.5">{publishedQuizzes.length}</p>
        </div>
      </div>
    </div>
      ...
      <div className="lg:col-span-5 space-y-6">
        <div className="glass-card bg-slate-900/60 rounded-[32px] border border-white/10 p-6 lg:p-8 shadow-sm flex flex-col space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-white/10">
            ...
            <button type="button" onClick={onGenerateAI} disabled={isGeneratingQuiz}
          ...
          <form onSubmit={onPublish} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Exam Title</label>
              <input type="text" value={pubTitle} onChange={e => setPubTitle(e.target.value)} placeholder="e.g. DBMS Unit 2 Mid-Sem Assessment"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Duration (mins)</label>
                <input type="number" min={5} max={180} value={pubDuration} onChange={e => setPubDuration(Number(e.target.value))}
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1.5">Syllabus Unit</label>
              <input type="text" value={pubUnit} onChange={e => setPubUnit(e.target.value)} placeholder="e.g. Unit 1: Fundamentals"
                className="w-full bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500/40" />
            </div>
            ...
            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Question Builder ({pubQuestions.length})</span>
                {pubQuestions.length > 0 && (
                  <button type="button" onClick={() => setPubQuestions([])} className="text-[10px] font-bold text-rose-400 hover:text-rose-300 uppercase">Clear Questions</button>
                )}
              </div>
              <input type="text" value={qText} onChange={e => setQText(e.target.value)} placeholder="Enter question text..."
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={qOpt0} onChange={e => setQOpt0(e.target.value)} placeholder="Option A" className="bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-lg px-3 py-1.5 text-xs outline-none" />
                <input value={qOpt1} onChange={e => setQOpt1(e.target.value)} placeholder="Option B" className="bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-lg px-3 py-1.5 text-xs outline-none" />
                <input value={qOpt2} onChange={e => setQOpt2(e.target.value)} placeholder="Option C" className="bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-lg px-3 py-1.5 text-xs outline-none" />
                <input value={qOpt3} onChange={e => setQOpt3(e.target.value)} placeholder="Option D" className="bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-lg px-3 py-1.5 text-xs outline-none" />
              </div>
              <div className="flex items-center gap-3">
                <label className="text-[11px] font-bold text-slate-400 shrink-0">Correct Answer:</label>
              ...
              <input type="text" value={qExplanation} onChange={e => setQExplanation(e.target.value)} placeholder="GTU Explanation / Reference..."
                className="w-full bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-xl px-3 py-2 text-xs font-medium outline-none" />
            ...
            {pubQuestions.length > 0 && (
              <div className="max-h-48 overflow-y-auto space-y-2 pr-1 no-scrollbar">
                {pubQuestions.map((q, idx) => (
                  <div key={q.id} className="p-3 bg-indigo-500/10 border border-indigo-500/20 rounded-xl text-xs flex justify-between items-start gap-2">
                    <div>
                      <p className="font-bold text-white">{idx + 1}. {q.question}</p>
                      <p className="text-emerald-400 text-[11px] font-medium mt-0.5">Ans: {q.options[q.correctAnswer]}</p>
                    </div>
                    <button type="button" onClick={() => setPubQuestions(prev => prev.filter(x => x.id !== q.id))}
            ...
            <button type="submit" disabled={isPublishingQuiz}
              ...
              {isPublishingQuiz ? <Loader2 className="w-4 h-4 animate-spin" /> : <GraduationCap className="w-4 h-4" />}
        ...
        <div className="glass-card bg-slate-900/60 rounded-[32px] border border-white/10 p-6 shadow-sm space-y-4">
          <div className="flex justify-between items-center border-b border-white/10 pb-3">
            ...
            <span className="bg-white/10 text-slate-300 px-2 py-0.5 rounded text-xs font-bold">{publishedQuizzes.length}</span>
          </div>
          <div className="space-y-3 max-h-64 overflow-y-auto no-scrollbar">
            {publishedQuizzes.map(quiz => (
              <div key={quiz.id} className="p-4 bg-white/5 rounded-2xl border border-white/10 flex justify-between items-center hover:bg-white/10 transition-all">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="px-2 py-0.5 bg-indigo-500/20 text-indigo-300 font-bold text-[10px] rounded uppercase">{quiz.subject}</span>
                    <span className="text-slate-400 text-[11px] font-medium">• {quiz.durationMinutes} mins</span>
                  </div>
                  <h5 className="font-bold text-white text-xs">{quiz.title}</h5>
                  <p className="text-[10px] text-slate-400 mt-0.5">{quiz.questions.length} questions • Created by {quiz.createdBy || 'Faculty'}</p>
                </div>
      ...
      <div className="lg:col-span-7 glass-card bg-slate-900/60 rounded-[32px] border border-white/10 p-6 lg:p-8 shadow-sm flex flex-col">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-white/10">
          <div>
            ...
            <p className="text-xs text-slate-400 font-medium">Exam attempt logs & student GTU readiness scores</p>
          </div>
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-48">
              <Search className="absolute left-3 top-2.5 w-3.5 h-3.5 text-slate-400" />
              <input type="text" value={gradebookSearch} onChange={e => setGradebookSearch(e.target.value)} placeholder="Search student..."
            ...
            <select value={gradebookSubjectFilter} onChange={e => setGradebookSubjectFilter(e.target.value)}
                ...
                return (
                  <tr key={row.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="py-3.5 px-3">
                      <span className="px-2 py-0.5 bg-white/10 rounded text-[10px] font-bold uppercase text-slate-300">{row.subject}</span>
                    </td>
                    <td className="py-3.5 px-3 max-w-[150px] truncate text-slate-300">{row.quizTitle}</td>
                    <td className="py-3.5 px-3 text-center font-mono font-bold text-white">{row.score}/{row.totalQuestions}</td>
                    <td className="py-3.5 px-3 text-center">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-black ${
                        row.percentage >= 80 ? 'bg-emerald-500/20 text-emerald-400' :
                        row.percentage >= 60 ? 'bg-indigo-500/20 text-indigo-400' : 'bg-rose-500/20 text-rose-400'
                      }`}>
                        {row.percentage}%
                      </span>
                    </td>
                    <td className="py-3.5 px-3 text-center font-mono text-slate-400">{Math.floor(row.timeSpentSeconds / 60)}m {row.timeSpentSeconds % 60}s</td>
                    <td className="py-3.5 px-3 text-center">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        gtuReadiness >= 75 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border border-amber-500/20'
                      }`}>
                        {gtuReadiness}% Ready
                      </span>
                    </td>
