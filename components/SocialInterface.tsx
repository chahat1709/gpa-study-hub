...
const SocialInterface: React.FC = () => {
  ...
  const parseParticipants = (chat: ApiChat): string[] => {
    try {
      return typeof chat.participants === 'string'
  ...
  return (
    <div className="h-full flex flex-col lg:flex-row bg-slate-900/60 backdrop-blur-xl relative overflow-hidden text-white">
      {/* Sidebar */}
      <div
        className={`${activeChatId ? 'hidden lg:flex' : 'flex'} w-full lg:w-[350px] bg-[rgba(15,23,42,0.6)] border-r border-white/10 flex-col shrink-0 h-full overflow-hidden`}
      >
        <div className="p-6 bg-white/5 border-b border-white/10">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-bold font-display text-xl text-white tracking-tight">
                Campus Link
              </h2>
            ...
            <button
              ...
              className="stitch-btn p-3 min-h-[44px] min-w-[44px] flex items-center justify-center font-bold rounded-[12px] text-white transition-colors shadow-lg active:scale-95"
            >
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-[12px] text-sm outline-none focus:ring-2 focus:ring-stitch-cyan/30 transition-all font-medium text-white placeholder:text-slate-400"
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar py-2">
          {chats.map(chat => {
            ...
            const isOnline = participants.some(
              (p: string) => p !== currentUser?.id && onlineUsers.has(p)
            );
            return (
              <div
                key={chat.id}
              ...
              >
                <div className="relative">
                  <div className="w-12 h-12 bg-white/10 text-slate-200 rounded-xl flex items-center justify-center shrink-0 font-bold text-sm border border-white/10">
                    {chat.is_group ? (
                      <Users
                        className={`w-6 h-6 ${activeChatId === chat.id ? 'text-stitch-primary' : 'text-slate-400'}`}
                      />
                    ) : (
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex justify-between items-baseline mb-1">
                    <h3 className="font-bold font-display text-white truncate text-sm tracking-tight">
                      {otherName}
                    </h3>
      ...
      {activeChat ? (
          ...
          <div className="h-16 border-b border-white/10 flex justify-between items-center px-6 shrink-0 bg-slate-950/80 backdrop-blur-md">
            <div className="flex items-center gap-3">
              <button
                ...
                className="p-3 -ml-2 text-slate-300 lg:hidden min-h-[44px] min-w-[44px] flex items-center justify-center"
              >
              ...
              <div className="w-10 h-10 bg-gradient-to-br from-stitch-primary to-stitch-secondary rounded-lg flex items-center justify-center text-slate-950 shrink-0 font-bold text-sm shadow-[0_0_10px_rgba(195,192,255,0.3)]">
                {activeChat.is_group ? (
                  ...
                  getInitials(getOtherParticipantName(activeChat))
                )}
              </div>
              <div>
                <h2 className="font-bold font-display text-white leading-none text-sm tracking-tight">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="p-3 text-slate-400 hover:text-rose-400 hover:bg-white/10 rounded-lg transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                title="Report"
              >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-transparent no-scrollbar">
            ...
            {activeMessages.map(msg => {
              ...
              if (msg.sender_id === 'system')
                return (
                  <div key={msg.id} className="text-center py-4">
                    <span className="text-[11px] font-bold uppercase tracking-widest text-slate-300 bg-white/10 px-4 py-1.5 rounded-full border border-white/10">
                      {msg.content}
                    </span>
                  </div>
                );
              ...
              return (
                ...
                >
                  <div
                    className={`max-w-[80%] rounded-2xl px-5 py-3.5 shadow-sm border ${isMe ? 'bg-stitch-primary-container text-white border-stitch-primary rounded-br-none shadow-[0_0_15px_rgba(67,56,202,0.2)] font-medium' : 'glass-card bg-[rgba(15,23,42,0.6)] text-white border-white/10 rounded-bl-none'}`}
                  >
                    {!isMe && (
                      <p className="text-[11px] font-bold uppercase tracking-widest mb-1.5 text-slate-300">
                        {msg.sender_name}
                      </p>
                    )}
                    <EncryptedMessage
                    ...
                    />
                    <div
                      className={`mt-2.5 flex items-center gap-1.5 opacity-60 ${isMe ? 'justify-end text-slate-900' : 'justify-start'}`}
                    >
            ...
            {typingNames.length > 0 && (
              <div className="flex items-center gap-2 text-slate-400">
                <div className="flex gap-1">
                  <div
                    className="w-2 h-2 bg-stitch-cyan rounded-full animate-bounce"
        ...
        <div className="flex-1 hidden lg:flex flex-col items-center justify-center text-slate-300 bg-transparent">
          ...
          <p className="font-bold font-display text-slate-300 uppercase tracking-widest text-[10px]">
            Campus Communications Online
          </p>
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[10px] text-slate-500 font-bold">
              {connected ? 'Real-time messaging active' : 'Offline mode'}
            </span>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="stitch-btn mt-8 px-10 py-4 min-h-[44px] rounded-[16px] font-bold uppercase tracking-widest text-[11px] shadow-[0_0_20px_rgba(67,56,202,0.25)] transition-all active:scale-95 flex items-center justify-center text-white"
          >
            Initiate Connection
          </button>
        </div>
          ...
          <div className="glass-card bg-slate-900/80 rounded-[32px] shadow-2xl w-full max-w-md overflow-hidden animate-fade-in border border-white/10 text-white">
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
              ...
              <button
                ...
                className="text-slate-400 p-3 min-h-[44px] min-w-[44px] flex items-center justify-center rounded-xl hover:bg-white/10 hover:text-white border border-transparent transition-all"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-white/5 border border-white/10 rounded-xl outline-none font-bold text-sm text-white placeholder:text-slate-400 focus:ring-2 focus:ring-stitch-cyan/30 transition-all"
                  />
                </div>
                <div className="space-y-2 max-h-[350px] overflow-y-auto no-scrollbar py-2">
                  {isSearching ? (
                    <div className="flex justify-center py-12">
                      <Loader2 className="animate-spin text-stitch-cyan w-8 h-8" />
                    </div>
                  ) : (
                    searchResults.map(u => (
                      <div
                        key={u.id}
                        className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-2xl hover:border-stitch-cyan/50 hover:bg-stitch-cyan/10 transition-all group cursor-pointer"
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-11 h-11 bg-white/10 border border-white/10 rounded-xl flex items-center justify-center text-slate-200 font-bold text-xs group-hover:bg-stitch-cyan group-hover:text-slate-950 group-hover:border-transparent transition-all">
                            {getInitials(u.name)}
                          </div>
                          <div>
                            <span className="font-bold font-display text-white text-sm tracking-tight block">
                              {u.name}
                            </span>
                            <span className="text-[10px] text-slate-400 font-bold">
                              {u.enrollment_number || ''} - {u.branch} {u.semester}
                            </span>
                          </div>
                        </div>
                        <button
                          onClick={() => handleStartChat(u)}
                          className="stitch-btn text-white px-5 py-2.5 min-h-[44px] flex items-center justify-center rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95"
                        >
                          Link
                        </button>
                      </div>
