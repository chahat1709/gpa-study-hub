...
const EncryptedMessage: React.FC<EncryptedMessageProps> = ({ content, chatId, isMe, isEncrypted = true, isAI }) => {
  ...
  return (
    <div className="relative group min-w-[80px]">
      <div className={`transition-all duration-300 ${isDecrypting ? 'opacity-40 blur-[1px]' : 'opacity-100 blur-0'}`}>
        <p className={`leading-relaxed whitespace-pre-wrap font-sans text-sm ${isDecrypting ? 'font-mono' : ''}`}>
          {displayText}
        </p>
      </div>
      
      <div className={`flex items-center gap-1.5 mt-2 transition-opacity duration-300 ${isDecrypting ? 'opacity-40' : 'opacity-30 group-hover:opacity-100'}`}>
        {isDecrypting ? (
          ...
          <>
            ...
            <span className="text-[10px] uppercase font-bold tracking-tighter">Hardware Verified E2EE</span>
          </>
        ) : (
          <div className="flex items-center gap-1">
