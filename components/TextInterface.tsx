...
const TextInterface: React.FC = () => {
  ...
  const presets = [
    { label: 'Summarize Notes', icon: FileText, prompt: 'Summarize the following notes into concise bullet points:' },
    { label: 'Create Quiz', icon: HelpCircle, prompt: 'Create 5 multiple choice questions with answers based on this text:' },
    { label: 'Glossary', icon: List, prompt: 'Extract key terms and definitions from the following text:' },
    { label: 'Simplify', icon: Sparkles, prompt: 'Explain the following concept like I am a beginner:' },
  ];
  ...
  return (
    <div className="h-full flex flex-col gap-6 max-w-4xl mx-auto p-4 lg:p-8 animate-fade-in">
      ...
      <div className={`flex-1 glass-card rounded-[40px] border border-white/10 text-white shadow-2xl flex flex-col transition-all duration-500 ${result || isLoading ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
        <div className="border-b border-white/10 p-6 flex justify-between items-center bg-white/5">
          <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-300">Neural Response</h3>
          <button
        ...
        <div className="p-8 flex-1 overflow-y-auto no-scrollbar">
          {isLoading ? (
            ...
            <div className="prose prose-invert max-w-none">
              <p className="whitespace-pre-wrap leading-relaxed text-slate-100 font-medium">{result}</p>
            </div>
          )}
        </div>
      </div>
