...
import { Clock, Award, CheckCircle2, XCircle, Sparkles, Check, Loader2 } from 'lucide-react';
        ...
        <div className="glass-card p-6 lg:p-8 rounded-3xl border border-white/10 text-white shadow-2xl flex flex-col gap-6">
          <div>
            ...
            <p className="text-slate-300 text-xs mt-1">
              Select your engineering subject and unit syllabus to generate a timed mock test.
            </p>
          </div>
    ...
    <div role="tabpanel" className="flex flex-col gap-6 animate-fade-in">
      <div className="glass-card rounded-3xl border border-white/10 text-white shadow-2xl flex flex-col overflow-hidden">
        <div className="p-4 lg:p-6 bg-slate-950/80 text-white flex justify-between items-center border-b border-white/10">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-stitch-primary block">
              {activeQuiz.subject}
            </span>
            <h3 className="text-base font-bold font-display text-white">{activeQuiz.title}</h3>
          </div>
          <div
            className={`px-4 py-2 rounded-xl text-sm font-mono font-black flex items-center gap-2 ${timeRemaining < 60 ? 'bg-red-500 text-white animate-pulse' : 'bg-white/10 text-stitch-cyan'}`}
          >
        ...
        {!isQuizSubmitted ? (
          <div className="p-6 lg:p-8 flex flex-col gap-6">
            <div className="flex justify-between items-center text-xs font-bold text-slate-400 uppercase">
              ...
              <span>
                Progress: {Math.round(((currentQIndex + 1) / activeQuiz.questions.length) * 100)}%
              </span>
            </div>
            <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden">
              <div
                className="bg-stitch-primary h-full transition-all duration-300"
              ...
              {activeQuiz.questions[currentQIndex]?.options.map((opt, oIdx) => {
                ...
                return (
                  ...
                  >
                    <span>{opt}</span>
                    <div
                      className={`w-5 h-5 rounded-full border flex items-center justify-center ${isSelected ? 'border-stitch-primary bg-stitch-primary text-white' : 'border-white/30'}`}
                    >
            ...
            <div className="flex justify-between items-center pt-4 border-t border-white/10">
              <button
                ...
                className="px-5 py-2.5 min-h-[44px] bg-white/10 text-slate-200 rounded-xl text-xs font-bold disabled:opacity-40 hover:bg-white/20"
              >
                Previous
              </button>
              {currentQIndex < activeQuiz.questions.length - 1 ? (
                <button
                  ...
                  className="stitch-btn px-6 py-2.5 min-h-[44px] text-white rounded-[12px] text-xs font-bold"
                >
                  Next Question
                </button>
              ) : (
          ...
          <div className="p-6 lg:p-8 flex flex-col gap-6 animate-fade-in">
            <div className="text-center bg-white/5 p-6 rounded-[24px] border border-white/10">
              ...
              <p className="text-xs text-slate-300 mt-1">
                You answered {quizScore?.score} out of {quizScore?.totalQuestions} correctly.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {activeQuiz.questions.map((q, idx) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="p-4 bg-white/5 border border-white/10 rounded-2xl text-xs space-y-2 text-white"
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {isCorrect ? (
                        <span className="px-2 py-0.5 bg-stitch-cyan/20 text-stitch-cyan font-bold border border-stitch-cyan/30 rounded-md flex items-center gap-1 shrink-0">
                          <CheckCircle2 className="w-3 h-3" /> Correct
                        </span>
                      ) : (
                        <span className="px-2 py-0.5 bg-red-500/20 text-red-400 font-bold border border-red-500/30 rounded-md flex items-center gap-1 shrink-0">
                    ...
                    <p className="text-slate-300">
                      Your Answer:{' '}
                      <strong className={isCorrect ? 'text-stitch-cyan' : 'text-red-400'}>
                        {userAns !== undefined ? q.options[userAns] : 'Not Answered'}
                      </strong>
                    </p>
                    <p className="text-stitch-cyan font-medium">
