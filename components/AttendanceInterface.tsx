  ...
  return (
    <div className="h-full flex flex-col bg-transparent text-white">
      ...
      <div className="bg-white/5 border-b border-white/10 px-6 py-4 flex justify-between items-center shrink-0">
        <div>
          <h1 className="text-xl font-bold font-display text-white tracking-tight">Attendance</h1>
        </div>
        <div className="flex bg-white/10 rounded-lg p-1" role="tablist">
          <button
            ...
            className={`px-4 py-2 min-h-[44px] rounded-[12px] text-xs font-bold transition-all flex items-center justify-center ${activeTab === 'OVERVIEW' ? 'stitch-btn text-white' : 'text-slate-300 hover:text-white'}`}
          >
            Stats
          </button>
          <button
            ...
            className={`px-4 py-2 min-h-[44px] rounded-[12px] text-xs font-bold transition-all flex items-center justify-center ${activeTab === 'TIMETABLE' ? 'stitch-btn text-white' : 'text-slate-300 hover:text-white'}`}
          >
            Schedule
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-6">
        {activeTab === 'OVERVIEW' && (
          <div role="tabpanel" className="max-w-4xl mx-auto space-y-6 animate-fade-in">
            ...
            {/* Subject Table */}
            <div className="glass-card bg-white/5 rounded-[24px] border border-white/10 text-white shadow-2xl overflow-hidden">
                  ...
                  >
                    ...
                    <div className="w-1/3 flex flex-col justify-center px-4">
                      <div className="w-full bg-white/10 h-2 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${sub.percentage >= 75 ? 'bg-stitch-cyan' : sub.percentage >= 60 ? 'bg-amber-400' : 'bg-rose-400'}`}
