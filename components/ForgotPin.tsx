...
export const ForgotPin: React.FC<ForgotPinProps> = ({ onBack, onSuccess }) => {
  ...
  const handleCheckEnroll = (e: React.FormEvent) => {
    e.preventDefault();
    if (!enrollment.trim()) { showError('Enter your enrollment number'); return; }
    setStep('NEWPIN');
  };

  const handleResetPin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!/^\d{4}$/.test(newPin)) { showError('PIN must be exactly 4 digits'); return; }
...
<label htmlFor="forgot-pin-enrollment" className="block text-[11px] font-semibold text-slate-400 mb-1.5 uppercase tracking-widest">
          ...
          <button type="submit"
            ...
            style={{ background: 'linear-gradient(135deg, #00f2fe, #10b981)', color: '#040810', boxShadow: '0 8px 24px rgba(0,242,254,0.25)' }}>
      ...
      {step === 'NEWPIN' && (
        <form onSubmit={handleResetPin} className="space-y-4">
          <div className="px-3 py-2.5 rounded-xl text-xs text-cyan-300 font-medium"
          ...
          {newPin.length === 4 && confirmPin.length === 4 && (
            <div className={`text-[11px] text-center font-bold ${newPin === confirmPin ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <button type="submit" disabled={isLoading}
            className="w-full py-3.5 min-h-[52px] rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98]"
            style={{ background: 'linear-gradient(135deg, #00f2fe, #10b981)', color: '#040810', boxShadow: '0 8px 24px rgba(0,242,254,0.25)' }}>
            {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <><KeyRound className="w-4 h-4" /> Reset PIN</>}
          </button>
        </form>
      )}
    </div>
