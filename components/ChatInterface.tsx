...
const SYSTEM_PROMPT =
  'You are a professional study tutor for GTU diploma students. Be helpful, concise, and academic. Use simple language students can understand.';
...
const ChatInterface: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages(prev => [
        ...prev,
        { id: modelMsgId, role: 'model', content: '', timestamp: Date.now(), isLoading: true },
      ]);
      ...
      setMessages(prev =>
        prev.map(m => (m.id === modelMsgId ? { ...m, content: response, isLoading: false } : m))
      );
    } catch {
    ...
    setMessages([
      {
        id: 'w',
        role: 'model',
        content: 'Memory buffer cleared. How can I help?',
        timestamp: Date.now(),
      },
    ]);
  };
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {messages.map(msg => (
            ...
            >
              ...
              >
                <p
                  className={`whitespace-pre-wrap ${msg.role === 'user' ? 'text-slate-950' : 'text-white'}`}
                >
                  {msg.content}
                </p>
                {msg.isLoading && (
                  <div className="flex gap-1 mt-1 opacity-70">
                    <span className="text-xs text-stitch-cyan font-bold font-display">
                      Thinking...
                    </span>
                  </div>
                )}
