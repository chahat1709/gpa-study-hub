        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-display-lg text-lg text-white tracking-tight leading-none">GPA Hub</h1>
            ...
            <p className="font-label-sm text-[11px] text-white/60 font-semibold uppercase tracking-wider mt-1">Academic Portal</p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/10">
        <div className="ui-user-panel flex items-center gap-3 transition-all cursor-pointer group hover:bg-white/5" onClick={() => fileInputRef.current?.click()}>
          <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 relative border-2 border-primary/30 shadow-md group-hover:border-primary transition-colors">
            ...
            ) : (<div className="w-full h-full flex items-center justify-center bg-surface"><UserIcon className="w-4 h-4 text-white/50" /></div>)}
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-label-md text-xs truncate text-white group-hover:text-primary transition-colors">{user?.name}</p>
            <p className="text-[11px] text-white/50 truncate font-mono">{user?.enrollmentNumber || user?.role}</p>
          </div>
          <button onClick={(e) => { e.stopPropagation(); logout(); }} className="p-3 min-h-[48px] min-w-[48px] text-white/50 hover:text-error transition-colors rounded-xl hover:bg-error/10 flex items-center justify-center" aria-label="Log out">
