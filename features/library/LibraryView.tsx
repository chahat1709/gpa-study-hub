...
import { FileText, Download, Search, ExternalLink, Video, ChevronRight, Globe, Loader2, Filter, Clock, Folder } from 'lucide-react';
    ...
    <div className="h-full flex flex-col bg-transparent text-white">
      ...
      <div className="shrink-0 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 px-6 py-3 sticky top-0 z-30">
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar max-w-7xl mx-auto">
          ...
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => { setSelectedCategory(cat); setResearchResult(null); }}
      ...
      <div className="flex-1 flex overflow-hidden max-w-7xl mx-auto w-full">
        ...
        <div className="w-64 bg-white/5 border-r border-white/10 shrink-0 hidden md:flex flex-col overflow-y-auto">
          <div className="p-4">
            <h3 className="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-3 px-2">Subjects</h3>
            <div className="space-y-1">
              {subjects.map((sub) => (
                <button
                  key={sub}
                  onClick={() => { setSelectedSubject(sub); setResearchResult(null); }}
             ...
             {researchResult ? (
               <div className="glass-card rounded-2xl p-8 border border-white/10 text-white shadow-2xl animate-fade-in">
                  ...
                  <div className="prose prose-invert max-w-none text-sm">
                     <p className="whitespace-pre-wrap leading-relaxed text-slate-100">{researchResult}</p>
                  </div>
                  <button onClick={() => setResearchResult(null)} className="mt-8 text-sm font-medium text-indigo-400 hover:text-indigo-300 p-3 min-h-[44px] flex items-center">← Back to Resources</button>
               </div>
             ) : resources.length > 0 ? (
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 {resources.map((res) => (
                   <div key={res.id} className="group p-5 rounded-xl border border-white/10 hover:border-indigo-400 hover:shadow-xl transition-all glass-card text-white flex flex-col">
                     <div className="flex items-start gap-4 mb-4">
                       <div className={`p-3 rounded-lg ${
                        res.type === 'pdf' ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 
                        res.type === 'video' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 
                       ...
                       <div className="flex-1 min-w-0">
                         ...
                         <h4 className="font-semibold text-white text-sm truncate mb-1 group-hover:text-indigo-400 transition-colors">{res.title}</h4>
                     ...
                     <div className="mt-auto pt-4 border-t border-white/10">
                        <a href={res.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-2.5 min-h-[44px] bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/30">
