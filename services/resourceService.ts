...
import { collection, query, where, getDocs, limit, addDoc, onSnapshot, deleteDoc, doc } from 'firebase/firestore';
...
export const resourceService = {
  getResources: (branch: string, semester: string, section: string, subject: string, category: string): LibraryResource[] => {
    if (isConfigValid) {
      let filtered = liveResources.filter(r =>
        (r.branch === branch) &&
        (r.semester === semester) &&
  ...
  uploadResource: async (
    ...
    subject: string, category: string, file: File,
    ...
    if (!allowedTypes.includes(file.type)) {
      throw new Error(`Security Violation: File type ${file.type} is not allowed. Only PDF, DOCX, and Images.`);
    }
    ...
    if (isConfigValid && db && storage) {
      const fileRef = ref(storage, `resources/${branch}/${semester}/${subject}/${Date.now()}_${file.name}`);
      ...
      return { ...resourceData, url, id: `pending_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` };
    } else {
